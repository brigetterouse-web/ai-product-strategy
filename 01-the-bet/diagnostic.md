...
Score rationale: The server build (in progress) minimises platform exposure because outcomes could be benchmarked against other users, and is therefore a value-add that Anthropic/Google/OpenAI couldn't ship. However, this hasn't been completed and relies on multiple users.
...
M — ~65% if we can ship the server, L - ~25% if it stays as is
